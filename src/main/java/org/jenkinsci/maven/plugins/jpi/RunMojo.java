...
public class RunMojo extends AbstractJetty6Mojo {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        // general-purpose system property so that we can tell from Jenkins if we are running in the jpi:run mode.
        setSystemPropertyIfEmpty("hudson.jpi.run","true");
        ...
        try {
            for( MavenArtifact a : getProjectArtfacts() ) {
                ...
                // find corresponding .jpi file
                Artifact jpi = artifactFactory.createArtifact(a.getGroupId(),a.getArtifactId(),a.getVersion(),null,"jpi");
                artifactResolver.resolve(jpi,getProject().getRemoteArtifactRepositories(), localRepository);
                ...
                FileUtils.writeStringToFile(new File(pluginsDir,a.getArtifactId()+".jpi.pinned"),"pinned");
            }
        } catch (IOException e) {
    ...
    public void finishConfigurationBeforeStart() {
        ...
        // to allow the development environment to run multiple "mvn jpi:run" with different port,
